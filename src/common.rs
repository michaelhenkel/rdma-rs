...
pub fn rdma_recv_md(id: *mut rdma_cm_id, mr: *mut ibv_mr, addr: *mut c_void, length: usize) -> anyhow::Result<(), CustomError>{
...
unsafe fn send_complete(id: MyId, iterations: usize) -> anyhow::Result<i32, CustomError>{
    let mut ret: c_int;
    ...
    loop {
        ret = ibv_poll_cq((*id.0).send_cq, BATCH_SIZE as i32, wc_ptr);
        if ret < 0 {
        ...
        println!("1 ret: {}, total_wc: {}",ret,  total_wc);
...
unsafe fn recv_complete(id: MyId, iterations: usize) -> anyhow::Result<i32, CustomError>{
    let mut ret: c_int;
    let mut cq = ptr::null::<ibv_cq>() as *mut _;
    let mut context = ptr::null::<c_void>() as *mut _;
